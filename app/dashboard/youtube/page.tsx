...
import { deleteYoutube } from "./actions";
      ...
      <div style={{ background: "#fffef9", border: "1px solid #e0d8cc", borderRadius: "14px", padding: "24px 28px" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" as const, fontSize: "13px" }}>
          ...
          <tbody>
            ...
            {items?.map((item) => {
              const del = deleteYoutube.bind(null, item.id);
              return (
                <tr key={item.id}>
                  <td style={tdStyle}>
                    <a href={item.url} target="_blank" rel="noopener noreferrer" style={{ color: "#0e7566", wordBreak: "break-all" as const }}>
                      {item.url}
                    </a>
                  </td>
                  ...
                  <td style={{ ...tdStyle, whiteSpace: "nowrap" as const }}>
                    <Link href={`/dashboard/youtube?edit=${item.id}`} style={{ fontSize: "12px", color: "#0e7566", marginRight: "12px" }}>
                      Edit
                    </Link>
