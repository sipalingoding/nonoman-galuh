...
export default function SidebarNav({ email }: { email: string }) {
  ...
  return (
    <>
      ...
      <div className="dashboard-mobile-bar">
        <Link href="/dashboard" style={{ textDecoration: "none" }}>
          <span
            ...
            style={{ fontSize: "16px", color: "#fffef9", fontWeight: 400 }}
          >
            Nonoman Galuh
          </span>
        </Link>
      ...
      {/* Backdrop */}
      <div
        className={`dashboard-sidebar-overlay${open ? " backdrop-open" : ""}`}
        onClick={() => setOpen(false)}
      />

      {/* Sidebar */}
      <aside
        className={`dashboard-sidebar-el${open ? " sidebar-open" : ""}`}
        style={{
          width: "260px",
          flexShrink: 0,
      ...
      >
        ...
        {/* Nav */}
        <nav className="dashboard-sidebar-nav" style={{ padding: "12px", flex: 1 }}>
          {menuItems.map((item) => {
