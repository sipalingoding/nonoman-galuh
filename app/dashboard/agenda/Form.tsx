...
function parseRangeToISO(tanggal: string) {
  const parts = tanggal.split(" s/d ");
  if (parts.length === 2) return { mulai: parseToISO(parts[0].trim()), selesai: parseToISO(parts[1].trim()) };
  return { mulai: parseToISO(tanggal.trim()), selesai: "" };
}
...
export default function AgendaForm({ editItem }: { editItem: any | null }) {
  ...
  return (
    <div style={{ marginBottom: "32px" }}>
      {!show && <button onClick={() => setShow(true)} style={primaryBtn}>+ Tambah Agenda</button>}
      {show && (
        <form
          ref={formRef}
          onSubmit={(e) => {
            ...
            startTransition(async () => {
              if (editItem) await updateAgenda(editItem.id, fd);
              else await createAgenda(fd);
        ...
        >
          ...
          <div>
            <label style={labelStyle}>Nama Agenda</label>
            <input name="nama" required defaultValue={editItem?.nama ?? ""} style={inputStyle} />
          </div>
          <div>
            <label style={labelStyle}>Deskripsi</label>
            <textarea name="deskripsi" required rows={3} defaultValue={editItem?.deskripsi ?? ""} style={{ ...inputStyle, resize: "vertical" }} />
          </div>
          <div>
            <label style={labelStyle}>Lokasi</label>
            <input name="lokasi" required defaultValue={editItem?.lokasi ?? ""} style={inputStyle} />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            <div>
              <label style={labelStyle}>Tanggal Mulai</label>
              <input name="tanggal_mulai" type="date" required defaultValue={existingDates.mulai} style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Tanggal Selesai</label>
              <input name="tanggal_selesai" type="date" defaultValue={existingDates.selesai} style={inputStyle} />
            </div>
          </div>
          <div>
