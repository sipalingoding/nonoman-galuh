      ...
      <div style={{ background: "#fffef9", border: "1px solid #e0d8cc", borderRadius: "14px", padding: "24px 28px" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" as const, fontSize: "13px" }}>
          ...
          <tbody>
            ...
            {items?.map((item) => {
              const del = deleteArtikel.bind(null, item.id);
              return (
                <tr key={item.id}>
                  <td style={tdStyle}>
                    {item.judul.length > 50 ? item.judul.slice(0, 50) + "…" : item.judul}
                  </td>
                  <td style={tdStyle}>{item.penulis}</td>
                  <td style={tdStyle}>{item.tanggal}</td>
                  <td style={{ ...tdStyle, whiteSpace: "nowrap" as const }}>
                    <Link href={`/dashboard/artikel?edit=${item.id}`} style={{ fontSize: "12px", color: "#0e7566", marginRight: "12px" }}>
                      Edit
                    </Link>
