        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "6px", flex: 1 }}>
          ...
          {item.ringkasan && (
            <p
              ...
              style={{ fontSize: "13px", color: "#7a6a54", lineHeight: 1.6, margin: 0, fontWeight: 300 }}
            >
              {item.ringkasan.length > 120 ? item.ringkasan.slice(0, 120) + "…" : item.ringkasan}
            </p>
          )}
        </div>
      </Link>
...
export default async function BeritaPage() {
  ...
  const { data, count } = await supabase
    .from("berita")
    .select("id, judul, ringkasan, tanggal, slug, gambar_url", { count: "exact" })
    .order("created_at", { ascending: false });

  const items: BeritaItem[] = data ?? [];
      ...
      <section className="px-sec" style={{ background: "#faf4e8", paddingTop: "56px", paddingBottom: "48px" }}>
        <div style={{ maxWidth: "1010px", margin: "0 auto" }}>
          ...
          <div className="ui-font" style={{ fontSize: "12px", color: "#8a7962", marginBottom: "20px", display: "flex", alignItems: "center", gap: "6px" }}>
            ...
            <span>›</span>
            <span style={{ color: "#4a3f30" }}>Berita & Informasi</span>
          </div>
          ...
          <div style={{ borderBottom: "1px solid rgba(209,78,31,.3)", paddingBottom: "28px" }}>
            <h1
              className="decorative-font"
              style={{ fontSize: "36px", fontWeight: 400, color: "#6f5f4c", marginBottom: "10px", lineHeight: 1.2 }}
            >
              Berita dan Informasi
            </h1>
            <p
              className="serif-title"
              style={{ fontSize: "14.22px", color: "#7a6a54", lineHeight: 1.55, fontWeight: 300, maxWidth: "600px" }}
            >
            ...
            {count != null && count > 0 && (
              <p className="ui-font" style={{ fontSize: "12px", color: "#8a7962", marginTop: "10px" }}>
                {count} berita tersedia
              </p>
            )}
          </div>
        </div>
        ...
        <div style={{ maxWidth: "1010px", margin: "0 auto" }}>
          {items.length > 0 ? (
            ...
            <div style={{ textAlign: "center", paddingTop: "60px", paddingBottom: "60px" }}>
              <p
                className="serif-title"
                style={{ fontSize: "16px", color: "#8a7962", fontWeight: 300 }}
              >
